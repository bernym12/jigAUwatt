...
class TasmotaDevice():
    ...
    async def getPower(self, output: t.PowerOutputType = t.PowerOutputType.OUTPUT_1) -> bool:
        ...
        if response.get(f'POWER{output}') != 'ON' and response.get(f'POWER{output}') != 'OFF': raise CommandError(f'Command failed: {response}')
        return response.get(f'POWER{output}') == 'ON'
    ...
    async def setPower(self, value: t.PowerType, output: t.PowerOutputType = t.PowerOutputType.OUTPUT_1) -> bool:
        ...
        if response.get(f'POWER{output}') is None: raise CommandError(f'Command failed: {response}')
        if value is t.PowerType.OFF:
            if response.get(f'POWER{output}') != 'OFF': raise CommandError(f'Command failed: {response}')
            return False
        elif value is t.PowerType.ON:
            if response.get(f'POWER{output}') != 'ON': raise CommandError(f'Command failed: {response}')
            return True
        elif value is t.PowerType.TOGGLE:
            if response.get(f'POWER{output}') != 'ON' and response.get(f'POWER{output}') != 'OFF': raise CommandError(f'Command failed: {response}')
            return response.get(f'POWER{output}') == 'ON'
        elif value is t.PowerType.BLINK:
            if response.get(f'POWER{output}') != 'Blink ON': raise CommandError(f'Command failed: {response}')
            return True
        elif value is t.PowerType.BLINK_OFF:
            if response.get(f'POWER{output}') != 'Blink OFF': raise CommandError(f'Command failed: {response}')
            return True
